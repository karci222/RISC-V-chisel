...
"h_000880e7".asUInt(32.W)
   ...
   val instructionMemory = Module(new DataMemory())
   

   
   flash.io.addrIn := pipeline.io.pcOut(15,0)
   instructionMemory.io.addrIn := pipeline.io.pcOut(15,0)
   instructionMemory.io.dataIn := 0.U
   instructionMemory.io.load := false.B
   pipeline.io.instrIn := "h_13".asUInt(32.W)

   when(pipeline.io.pcOut(31, 16) === 0.U){
      pipeline.io.instrIn := flash.io.dataOut
   }.elsewhen(pipeline.io.pcOut(31,16) === 1.U){
      pipeline.io.instrIn := instructionMemory.io.dataOut
   }
